...
function setup() {
  ...
  restart = createSprite(width/2,height/2+60,10,10)
  ...
  solo = createSprite(width/2,height-40,400,20);
  solo.addImage("ground",imagemdosolo);
  solo.x = solo.width /2;
 
  
  soloinvisivel = createSprite(50,height-32,400,10);
...
function draw() {
  ...
  if(touches.lenght>0||keyDown("space")&& trex.y >= solo.y-30) {
    trex.velocityY = -13;
    estadojogo = "jogar";
    somdepular.play()
    touches = []
...
function gerarObstaculos(){
 if (frameCount % 60 === 0){
   var obstaculo = createSprite(width,height-50,10,40);
...
function gerarNuvens() {
  ...
  if (frameCount % 60 === 0) {
    ...
    nuvem.y = Math.round(random(height-150,height-200));
